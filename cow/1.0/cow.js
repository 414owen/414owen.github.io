...
(function() {
	...
	var ws = /^\s*$/;
	var space = " ";
	var repSpace = rep.bind(space, space);
	...
	function cow(a) {
		...
		return rep("\n",3) + space + rep("_", longest + 2) + "\n" +
			split.map(function(b, i) {
				return sides[i * 2] + space + b + repSpace(longest - length(b)) + 
					space + sides[i * 2 + 1] + "\n";
			}).join("") +
			space + rep("-", longest + 2) + "\n" +
			repSpace(8)  + "\\   ^__^\n" +
	...
	document.onkeydown = function(e) {
		...
		if (e.keyCode === 8) {
			...
			// Support text input elements
			if (["textarea", "input"].indexOf(elName) !== -1) {
				prevent = false;
			}
